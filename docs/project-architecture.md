...
중요한 점은 실제 영상/오디오/이미지/폰트 파일 자체는 JSON에 저장되지 않는다는 것입니다. JSON에는 “이 파일이 필요하다”는 메타 정보와 편집 데이터만 저장합니다. 같은 브라우저에서는 영상과 가져온 폰트를 IndexedDB에서 자동 복구할 수 있지만, 다른 기기나 저장공간 제한 상황에서는 원본 파일을 다시 연결하거나 폰트를 다시 가져와야 할 수 있습니다.
...
- IndexedDB: 가능하면 원본 영상 파일과 가져온 폰트 파일을 로컬 보관

브라우저 저장공간 제한이 있거나 다른 컴퓨터에서 열면 원본 미디어 파일은 다시 연결해야 하며, 폰트도 다시 가져와야 할 수 있습니다.
